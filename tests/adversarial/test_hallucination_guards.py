...
class TestHallucinationGuards:
    ...
    def test_rejects_fictional_compound(self, agent, adversarial_scenarios):
        ...
        scenario = next(
            s for s in adversarial_scenarios["scenarios"] if s["id"] == "adv_fictional_compound"
        )
    ...
    def test_rejects_negative_tire_age(self, agent, adversarial_scenarios):
        ...
        scenario = next(
            s for s in adversarial_scenarios["scenarios"] if s["id"] == "adv_negative_tire_age"
        )
    ...
    def test_rejects_impossible_tire_age(self, agent, adversarial_scenarios):
        ...
        scenario = next(
            s for s in adversarial_scenarios["scenarios"] if s["id"] == "adv_impossible_tire_age"
        )
    ...
    def test_rejects_future_lap(self, agent, adversarial_scenarios):
        ...
        scenario = next(
            s for s in adversarial_scenarios["scenarios"] if s["id"] == "adv_future_lap"
        )
    ...
    def test_handles_slicks_in_heavy_rain(self, agent, adversarial_scenarios):
        ...
        scenario = next(
            s for s in adversarial_scenarios["scenarios"] if s["id"] == "adv_slicks_in_monsoon"
        )
        ...
        assert (
            recommendation["confidence_score"] >= expected["confidence_minimum"]
    ...
    def test_handles_wets_on_dry_track(self, agent, adversarial_scenarios):
        ...
        scenario = next(
            s for s in adversarial_scenarios["scenarios"] if s["id"] == "adv_wets_on_dry_track"
        )

        recommendation = agent.generate_strategy(scenario)
        expected = scenario["expected_behavior"]

        assert recommendation["decision"] == "BOX", "Must recommend pitting for WETs on dry track"
...
class TestConfidenceCalibration:
    ...
    def test_high_confidence_for_clear_decisions(self, agent):
        ...
        recommendation = agent.generate_strategy(scenario)
        assert recommendation["confidence_score"] >= 0.85, "Fresh tires should have high confidence"
